...
impl DataBrokerClient {
    ...
    /// Subscribe to signal changes on the given signal path.
    ///
        ...
        match self.client.get_value(request).await {
            Ok(response) => {
                ...
                match dp.and_then(|dp| dp.value).and_then(|v| v.typed_value) {
                    ...
                    Some(kuksa::val::v2::value::TypedValue::Float(f)) => {
                        Ok(Some(SignalValue::Float(f)))
                    }
    ...
    /// Write a signal value to DATA_BROKER.
    pub async fn set_signal(
        ...
        let typed_value = match value {
            ...
            SignalValue::Float(f) => kuksa::val::v2::value::TypedValue::Float(f),
