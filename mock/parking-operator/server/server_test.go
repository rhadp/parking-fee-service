...
// ---------------------------------------------------------------------------
// TS-09-P4: Parking Operator Session Uniqueness (sequential)
...
// Property 5 from design.md
// Requirement: 09-REQ-8.2, 09-REQ-8.5
// Description: For any number of concurrent start requests, all generated
// session_ids are unique UUIDs.
// ---------------------------------------------------------------------------
...
func TestConcurrentSessionUniqueness(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		go func(idx int) {
			defer wg.Done()
			body := fmt.Sprintf(
				`{"vehicle_id":"V%d","zone_id":"z1","timestamp":%d}`, idx, idx+1,
			)
			resp, err := http.Post(ts.URL+"/parking/start", "application/json",
				strings.NewReader(body))
			if err != nil {
			...
			if err := json.Unmarshal(data, &parsed); err != nil {
				results[idx] = result{err: err, status: resp.StatusCode}
				return
			}

			results[idx].sessionID, _ = parsed["session_id"].(string)
		}(i)
	}
	...
	for i, r := range results {
		...
		if r.status != http.StatusOK {
			t.Errorf("request %d: expected 200, got %d", i, r.status)
			continue
		}
		if !uuidRegex.MatchString(r.sessionID) {
			t.Errorf("request %d: %q is not a valid UUID", i, r.sessionID)
		}
		if ids[r.sessionID] {
			t.Errorf("request %d: duplicate session_id %q", i, r.sessionID)
		}
		ids[r.sessionID] = true
	}

	if len(ids) != n {
		t.Errorf("expected %d unique session IDs, got %d", n, len(ids))
	}
}
...
func TestSessionIntegrityProperty(t *testing.T) {
	...
	testCases := []struct {
		...
		{0, 7200, 7200, 5.00},                          // from epoch
		{1700000000, 1700036000, 36000, 25.00},         // 10 hours
		{1700000000, 1700090000, 90000, 62.50},         // 25 hours
		{1700000000, 1700000900, 900, 0.625},           // 15 minutes
		{1700000000, 1700014400, 14400, 10.00},         // 4 hours
	}
