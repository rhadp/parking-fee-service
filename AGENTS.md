...
3. **Read `.agent-fox/specs/steering.md`** if it exists — project-level directives that
...
2. Changes are committed with a clear conventional commit message.
   - **Always include tracked state files** in the commit:
     `.agent-fox/memory.jsonl` and `docs/memory.md`. These are
     git-tracked and must not be left dirty.
