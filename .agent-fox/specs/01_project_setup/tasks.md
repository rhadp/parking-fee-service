...
- [x] 3. Implement skeleton binaries and placeholder tests
  ...
  - [x] 3.V Verify: run `cargo test --workspace` in `rhivos/` and `go test ./...` from repo root; confirm all tests pass
    ...
    _Note: 5 pre-existing spec 09 sensor_tests.rs failures remain (see docs/errata/01_skeleton_vs_spec09_sensors.md). All spec 01 tests pass._
...
- [x] 7. Wiring verification
  ...
  - [x] 7.2 Run proto generation and verify Go integration: `make proto && make build-go`
    _Test Spec: TS-01-SMOKE-3_
    _Requirements: 01-REQ-10.1, 01-REQ-10.3_
    _Note: `go build ./...` from repo root does not work in a Go workspace without a root module; `make build-go` (which runs `go build ./...` per module) is the correct equivalent._
  ...
  - [x] 7.5 Verify all skeleton binaries produce version output (manual or scripted)
    ...
    _Note: All 7 Rust binaries and 5 Go binaries print "{component-name} v0.1.0" and exit 0. Covered by TestRustSkeletonVersionStrings and TestGoSkeletonVersionStrings in tests/setup/_
