...
- **C6 (Standard VSS signals):** The Kuksa Databroker image bundles a standard VSS release tree (e.g., `vss_release_4.0.json`) but does not load it automatically. The compose.yml must explicitly pass both the standard VSS tree and the custom overlay via the `--vss` flag (e.g., `--vss /vss_release_4.0.json,/app/vss-overlay.json`) so that all 5 standard signals and 3 custom signals are available. Without the standard tree, only custom overlay signals would be loaded.
- **C7 (Image version):** Pin to `ghcr.io/eclipse-kuksa/kuksa-databroker:0.5.0` for reproducibility. Tag `:0.5.1` does not exist in the container registry; `:0.5.0` is the latest available release. The version can be updated via compose.yml as needed.
...
- **C9 (VSS overlay completeness):** The overlay must include not only the 3 custom leaf signals but also the intermediate branch nodes (`Vehicle.Parking`, `Vehicle.Command`, `Vehicle.Command.Door`) with `type: "branch"`. The Kuksa Databroker flat JSON format requires explicit branch definitions for custom signal paths that are not part of the standard VSS tree. Without these branch nodes, the leaf signals will fail to load.
