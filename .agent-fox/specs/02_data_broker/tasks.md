...
- [x] 2. Configure compose.yml for dual listeners
  ...
  - [x] 2.V Verify task group 2
    ...
    - [x] tests/databroker added to GO_TEST_MODULES in Makefile; `make test` passes with databroker tests included
...
- [x] 3. Validate VSS overlay
  ...
  - [x] 3.V Verify task group 3
    - [x] Static tests pass: `TestVSSOverlayFormat` verifies all 3 custom signals present with correct types, branch nodes defined, and JSON valid (20 subtests pass including leaf_type, datatype, not_branch, description checks)
    - [x] `TestVSSOverlayNoDuplicateKeys` passes: no duplicate JSON keys found at any nesting level
    - [x] `make check` passes: all quality gates green
    ```
...
- [x] 4. Implement edge case tests
  ...
  - [x] 4.4 Implement test for permissive mode with arbitrary token (expect success)
    ...
    - `TestPermissiveModeWithArbitraryToken` in tests/databroker/edge_test.go; dials TCP with insecure credentials, adds `Authorization: Bearer invalid-token-12345` metadata, calls Get(Vehicle.Speed), asserts no PERMISSION_DENIED error; skips when TCP unreachable
  ...
  - [x] 4.V Verify task group 4
    - [x] All edge case tests compile and pass (SKIP when databroker unavailable; PASS for static checks and Podman-based tests)
