...
- [ ] 2. Download and configure Kuksa Databroker binary
  - [ ] 2.1 Add Kuksa Databroker to docker-compose
    - Add `databroker` service to the project's `docker-compose.yml`
    - Image: `ghcr.io/eclipse-kuksa/kuksa-databroker:latest` (pin tag once validated)
...
- [ ] 4. Configure dual listeners (UDS + TCP)
  ...
  - [ ] 4.2 Configure UDS listener
    - Add `--uds` flag or environment variable to enable UDS endpoint
    - Create shared volume `kuksa-uds` for the UDS socket path `/tmp/kuksa/databroker.sock`
    - Verify same-partition access via UDS
