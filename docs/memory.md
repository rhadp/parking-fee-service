...
- The COMPANION_APP is a Flutter/Dart project at `android/companion-app/` (package: `companion_app`, org: `com.rhadp`, applicationId: `com.rhadp.companion_app`). Communicates with CLOUD_GATEWAY via REST. Uses `provider` for state management, `http` package for HTTP, and `shared_preferences` for token persistence. Follows a modular structure: `models/`, `services/`, `providers/`, `screens/` under `lib/`.
...
- COMPANION_APP JSON conventions: CLOUD_GATEWAY fields use snake_case (e.g., `is_locked`, `command_id`, `parking_session_active`); Dart model properties use camelCase. Empty string `""` for `result` and `updated_at` fields must be treated as null during deserialization.
...
- We use `http.MockClient` (from `package:http/testing.dart`) for COMPANION_APP client tests (not mockito) because it's built-in to the http package and requires no code generation.
