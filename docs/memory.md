...
- The COMPANION_APP is a Flutter/Dart project at `android/companion-app/` (package: `companion_app`, org: `com.rhadp`, applicationId: `com.rhadp.companion_app`). Uses Provider/ChangeNotifier for state management, the `http` package with `MockClient` for testing.
...
- The Flutter COMPANION_APP uses `flutter_lints` (not the newer `flutter_lints` replacement); `analysis_options.yaml` includes `package:flutter_lints/flutter.yaml`.
