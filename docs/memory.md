...
- The project has five main technology domains: Go backend services (`backend/`), Rust RHIVOS services (`rhivos/`), Go mock CLI tools (`mock/`), the Android PARKING_APP (`android/parking-app/`), and the Flutter COMPANION_APP (`android/companion-app/`).
...
- The COMPANION_APP is a Flutter/Dart project at `android/companion-app/` (package: `companion_app`, org: `com.rhadp`, applicationId: `com.rhadp.companion_app`). Communicates with CLOUD_GATEWAY via REST. Uses `provider` for state management, `http` package for HTTP, and `shared_preferences` for token persistence. Follows a clean layered architecture: `models/` → `services/` → `providers/` → `screens/` under `lib/`, where each layer depends only on the one below it. `VehicleProvider` uses `ChangeNotifier` with the `provider` package for reactive state, wrapping `CloudGatewayClient` for REST calls and `SharedPreferences` for token persistence.
...
- The update-service manages adapter containers via podman CLI with JSON-persisted state and `tokio::sync::broadcast`-based `WatchAdapterStates` streaming for real-time state change notifications. When the adapter image isn't in the local podman registry, `InstallAdapter` still succeeds at the API level but the adapter enters ERROR state (5) instead of RUNNING (3).
...
- Go mock CLIs use env var defaults with CLI flag overrides via `envOrDefault()`. The companion-app-cli uses the `flag` package; the parking-app-cli uses manual flag parsing with `parseGlobalFlags` (no `flag` package). Both use kebab-case flags with `--` prefix (e.g., `--vehicle-vin`, `--image-ref`, `--adapter-id`).
- Go CLI subcommand handlers follow the pattern: validate required flags → make request → parse response → `printJSON(response)` to stdout; errors go to stderr and return non-nil. gRPC output uses `json.MarshalIndent` (proto messages serialize with snake_case field names and numeric enum values). REST responses are parsed as `json.RawMessage` then re-printed with `printJSON` for indented output, avoiding the need to define response structs in the CLI.
- Go test files in `mock/` use `httptest.NewRecorder()` and `httptest.NewRequest()` for handler testing (not `httptest.Server`). CLI tools use `httptest.NewServer` for mocking REST endpoints (e.g., PARKING_FEE_SERVICE) and `net.Listen` + `grpc.NewServer` on random ports for mocking gRPC services, verifying request construction and response handling without real services.
- Go test files in the parking-fee-service use `*_test.go` in the same package (white-box testing) to access internal fields like `store.zones`. Tests use `httptest.NewServer` with a real `http.ServeMux` (not mocked); test helpers like `testStore()` and `testServer()` create isolated instances.
...
- The Flutter COMPANION_APP uses `flutter_lints` (not the newer `flutter_lints` replacement); `analysis_options.yaml` includes `package:flutter_lints/flutter.yaml`. The `library;` directive at the top of `models.dart` is required by `flutter_lints`; removing it causes an analyzer warning.
- All COMPANION_APP Dart data models use `fromJson` factory constructors and `toJson` methods with snake_case JSON keys mapped to camelCase Dart fields. CLOUD_GATEWAY fields use snake_case (e.g., `is_locked`, `command_id`, `parking_session_active`). Empty string `""` for `result` and `updated_at` fields must be treated as null during deserialization.
- `GatewayException.fromResponse` takes `(int statusCode, String body)` — not the raw `http.Response` — to avoid leaking the `http` package dependency into models.
- Speed values from the CLOUD_GATEWAY API may arrive as integers; use `(json['speed'] as num?)?.toDouble()` for int-to-double coercion.
...
- E2E integration tests live in `tests/` at the project root (e.g., `tests/test_cloud_e2e.sh`). Makefile target `test-e2e` runs E2E tests; `test` runs unit tests only. Bash test scripts follow a consistent pattern: color helpers, pass/fail/skip counters, JSON parsing via python3, cleanup traps, infrastructure-availability checks that skip gracefully, and service startup with PID tracking.
...
- Integration test coordinates were chosen to exercise all three code paths: inside polygon (Marienplatz center), fuzzy match (slightly south of Olympiapark's lower edge at 48.1712), and no-match (Berlin coordinates).
...
- The parking-fee-service `healthz` endpoint returns `{}` (empty JSON object) rather than `{"status":"ok"}` — matches the design doc specification.
- Zone lookup returns `[]` (empty JSON array) rather than `null` when no matches — explicitly handled in the handler to ensure the store's nil slice becomes an empty slice.
- The `ZoneDetailResponse` struct uses `json.RawMessage` for the polygon field to avoid double-encoding; keeps the response flat.
...
- The COMPANION_APP keeps `minSdk = maxOf(flutter.minSdkVersion, 21)` instead of hardcoding `minSdk = 21` because Gradle's Flutter plugin upgrade process rewrites `build.gradle.kts` and reverts direct assignments; using `maxOf` survives the rewrite.
- The COMPANION_APP sets `android:usesCleartextTraffic="true"` in AndroidManifest.xml because the default gateway URL is HTTP (`http://10.0.2.2:8081`, per 07-REQ-5.2).
- The root `.gitignore` covers both Flutter and native Android artifacts with glob patterns like `**/.dart_tool/`, `**/.gradle`, `**/build/`.
...
- We use `http.MockClient` (from `package:http/testing.dart`) for COMPANION_APP client tests (not mockito) because it's built-in to the http package and requires no code generation. `CloudGatewayClient` accepts an injectable `http.Client` in its constructor for testability. Note: `MockClientHandler` typedef is exported from `package:http/testing.dart`, not from `package:http/http.dart`.
- Timer-based COMPANION_APP tests (e.g., VehicleProvider polling) use `fakeAsync` from `package:fake_async` to control time progression; see Decisions for why it's an explicit dep.
- `GatewayException` has a `userMessage` getter that maps HTTP status codes to user-friendly strings (403→"Wrong PIN", 404→"Vehicle not found") to centralize error display logic.
- Network errors (connection refused, timeout) are caught in the COMPANION_APP client and re-thrown as `GatewayException(statusCode: 0, code: 'NETWORK_ERROR')` to provide a uniform error type.
- We use `fake_async` as an explicit dev dependency (not re-exported from flutter_test) because flutter_test does not publicly export the `fakeAsync()` function, and the Dart analyzer requires it declared.
- COMPANION_APP command polling uses a `Completer` to make `_pollForCommandResult` awaitable, so `sendCommand` returns only after the result is received or timeout.
...
- The parking-app-cli's gRPC tests that connect to `localhost:1` take ~5 seconds each due to the `dialGRPC` connection timeout, making the full test suite take ~85 seconds. Consider shorter timeouts for test mode if performance becomes an issue.
- The `parseGlobalFlags` function in parking-app-cli processes all flags in a single pass; adding new global flags requires careful ordering if they could conflict with subcommand-specific flags (currently no conflicts exist).
...
- The fuzzy match test coordinate (48.1712, 11.5535) is ~89m from the Olympiapark polygon edge. Changing the seed data polygon coordinates in `zones/seed.go` would break this test expectation.
- UPDATE_SERVICE adapter state depends on whether the container image exists in the local podman registry. Tests must tolerate both RUNNING (3) and ERROR (5) states.
...
- The `build.gradle.kts` under `android/companion-app/android/app/` gets modified by `flutter build` (Gradle plugin upgrade). Manual edits may be overwritten; use expressions like `maxOf(flutter.minSdkVersion, N)` to persist custom values.
...
- The COMPANION_APP's `updated_at` field parsing assumes ISO 8601 format; if the backend changes the timestamp format, `DateTime.parse` will throw.
- COMPANION_APP timer-based tests are sensitive to the order of `async.elapse()` and `async.flushMicrotasks()` calls — always flush microtasks after elapsing time to let async HTTP handlers complete.
- `SharedPreferences.setMockInitialValues()` must be called before `SharedPreferences.getInstance()` in each COMPANION_APP test to ensure proper mock initialization.
