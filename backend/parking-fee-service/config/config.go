...
func LoadConfig(path string) (*model.Config, error) {
	...
	if err != nil {
		...
		// Other file errors (e.g. permission denied) also fall back to defaults
		// since they indicate the file is not accessible.
		slog.Warn("config file not accessible, using defaults", "path", path, "error", err)
		return DefaultConfig(), nil
	}
