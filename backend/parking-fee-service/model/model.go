...
// Coordinate represents a geographic latitude/longitude point.
...
// Rate represents a parking rate (per-hour or flat-fee).
type Rate struct {
...
// AdapterMeta holds information needed to download a PARKING_OPERATOR_ADAPTOR.
...
// OperatorResponse is the public representation of an Operator for lookup
// responses. The Adapter field is intentionally excluded.
...
// Config is the top-level configuration for the parking-fee-service.
type Config struct {
	Port               int        `json:"port"`
