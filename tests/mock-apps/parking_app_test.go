...
func TestInstall(t *testing.T) {
	...
	// a connection error (exit 1). It should NOT exit 0 without doing anything.
...
func TestList(t *testing.T) {
	...
	if exitCode == 0 && len(stdout) == 0 {
		t.Error("expected binary to produce output or fail with connection error")
	}
	_ = stderr
}
...
func TestAdapterStatus(t *testing.T) {
	...
	// The binary should recognize the subcommand and attempt gRPC connection.
	// With no server running, it should exit 1 with an error.
	// If a mock server is available, it should print the adapter status.
	if exitCode == 0 && !strings.Contains(stdout, "a1") {
		t.Errorf("exit 0 but adapter ID not in output\nstdout: %s\nstderr: %s", stdout, stderr)
	}
}
...
func TestStartSession(t *testing.T) {
	...
	// Should recognize the subcommand and attempt gRPC connection
	if exitCode == 0 && len(stdout) == 0 {
		t.Error("expected binary to produce output or fail with connection error")
	}
	_ = stderr
...
// Requirement: 09-REQ-5.E1 (install missing image-ref/checksum)
// ---------------------------------------------------------------------------
