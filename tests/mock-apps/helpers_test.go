...
func findRepoRoot(t *testing.T) string {
	...
	for {
		...
		// Also check for .git file (worktree case)
...
// waitUntilListening polls a URL until it responds or the timeout elapses.
func waitUntilListening(t *testing.T, baseURL string, timeout time.Duration) {
	t.Helper()
	deadline := time.Now().Add(timeout)
	client := &http.Client{Timeout: 200 * time.Millisecond}
	for time.Now().Before(deadline) {
		// Try a simple GET to the base URL (even if 404, it means the server is up).
		resp, err := client.Get(baseURL + "/parking/status/probe")
		if err == nil {
			resp.Body.Close()
			return
		}
	...
	t.Fatalf("server at %s did not start within %v", baseURL, timeout)
}

// doHTTPPost sends a POST request with JSON body.
func doHTTPPost(t *testing.T, url, body string) *http.Response {
	t.Helper()
...
// buildSensorBinary builds a named Rust sensor binary and returns its path.
func buildSensorBinary(t *testing.T, root, name string) string {
	t.Helper()
	...
	return filepath.Join(root, "rhivos", "target", "debug", name)
}
