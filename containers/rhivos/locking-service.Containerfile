# Containerfile for locking-service
#
...
RUN cd rhivos && cargo build --release --bin locking-service
...
COPY --from=builder /build/rhivos/target/release/locking-service /usr/local/bin/locking-service

EXPOSE 50051

ENTRYPOINT ["locking-service"]
