# Containerfile for cloud-gateway-client
#
...
RUN cd rhivos && cargo build --release --bin cloud-gateway-client
...
COPY --from=builder /build/rhivos/target/release/cloud-gateway-client /usr/local/bin/cloud-gateway-client

EXPOSE 50052

ENTRYPOINT ["cloud-gateway-client"]
