# Containerfile for update-service
#
...
RUN cd rhivos && cargo build --release --bin update-service
...
COPY --from=builder /build/rhivos/target/release/update-service /usr/local/bin/update-service

EXPOSE 50053

ENTRYPOINT ["update-service"]
