...
- [x] 1. Set up update-service project structure
  - [x] 1.1 Create Rust crate structure for update-service
  ...
  - [x] 1.2 Generate Rust bindings from update_service.proto
...
- [x] 2. Implement core data models and configuration
  - [x] 2.1 Implement AdapterState enum and AdapterEntry struct
...
- [x] 4. Implement operation logging
  - [x] 4.1 Implement OperationLogger struct
  ...
  - [x] 4.2 Write property test for request logging
  ...
  - [x] 4.3 Write property test for state transition logging
  ...
  - [x] 4.4 Write property test for container operation logging
    - **Property 24: Container Operation Logging**
    - Verify container operations are logged with outcome and correlation ID
    - **Validates: Requirements 12.3, 12.4**

- [x] 5. Implement registry authentication
  - [x] 5.1 Implement RegistryAuthenticator struct
  ...
  - [x] 5.2 Write property test for Bearer token authentication
...
- [x] 7. Implement image downloading
  ...
  - [x] 7.2 Write property test for download retry behavior
...
- [x] 8. Implement attestation validation
  - [x] 8.1 Implement AttestationValidator struct
  ...
  - [x] 8.2 Write property test for attestation verification
  ...
  - [x] 8.3 Write property test for attestation structure validation
...
- [x] 9. Implement container management
  - [x] 9.1 Implement ContainerManager struct
  ...
  - [x] 9.2 Write property test for container startup failure handling
...
- [x] 11. Implement state tracking
  - [x] 11.1 Implement StateTracker struct
  ...
  - [x] 11.2 Write property test for state timestamp updates
  ...
  - [x] 11.3 Write property test for list adapters completeness
...
- [x] 12. Implement watcher management
  - [x] 12.1 Implement WatcherManager struct
  ...
  - [x] 12.2 Write property test for watcher event reception
  ...
  - [x] 12.3 Write property test for watcher cleanup
  ...
  - [x] 12.4 Write property test for initial state emission
...
- [x] 13. Implement gRPC service
  - [x] 13.1 Implement UpdateServiceImpl struct
  ...
  - [x] 13.2 Implement InstallAdapter RPC handler
  ...
  - [x] 13.3 Write property test for install state initialization
  ...
  - [x] 13.4 Write property test for install idempotence (running)
  ...
  - [x] 13.5 Write property test for install idempotence (in-progress)
  ...
  - [x] 13.6 Write property test for invalid registry URL
  ...
  - [x] 13.7 Write property test for state progression
  ...
  - [x] 13.8 Implement UninstallAdapter RPC handler
    - Generate correlation ID and log request
  ...
  - [x] 13.9 Write property test for uninstall completeness
  ...
  - [x] 13.12 Implement WatchAdapterStates RPC handler
    - Generate correlation ID and log request
...
- [x] 14. Checkpoint - Verify gRPC service
  - Run `cargo test` for all tests
  - Verify service compiles and handlers are wired correctly
  - Ask the user if questions arise

- [x] 15. Implement gRPC server startup with TLS
  - [x] 15.1 Implement TLS server listener
...
- [x] 16. Implement offload scheduler
  - [x] 16.1 Implement OffloadScheduler struct
  ...
  - [x] 16.2 Write property test for automatic offload
...
- [x] 17. Checkpoint - Verify complete service
...
- [x] 18. Integration testing
  - [x] 18.1 Create mock registry for testing
  ...
  - [x] 18.2 Create mock container manager for testing
...
- [x] 19. Final checkpoint - Verify complete implementation
...
All 19 tasks completed with 77 tests passing (24 property tests covering all correctness properties from the design document).

**Implemented modules:**
- `lib.rs` - Library root exposing all modules
- `error.rs` - UpdateError enum with 14 variants and gRPC status mapping
- `config.rs` - ServiceConfig struct with environment variable support
- `state.rs` - AdapterEntry struct for state tracking
- `logger.rs` - OperationLogger with structured JSON logging
- `authenticator.rs` - RegistryAuthenticator with token caching and 401 handling
- `downloader.rs` - ImageDownloader with exponential backoff retry
- `attestation.rs` - AttestationValidator for container verification
- `container.rs` - ContainerManager for podman lifecycle operations
- `watcher.rs` - WatcherManager for streaming subscriptions
- `tracker.rs` - StateTracker for adapter state management
- `offload.rs` - OffloadScheduler for automatic cleanup
- `service.rs` - UpdateServiceImpl gRPC service handlers
- `test_utils.rs` - MockRegistry and test helpers
- `main.rs` - Full service startup with graceful shutdown
